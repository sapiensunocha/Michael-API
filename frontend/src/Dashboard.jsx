...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    async function fetchDashboardData() {
      ...
      setError(null);
      try {
        const profileData = await getPartnerProfile(user.token);
        setPartnerName(profileData.name || user.email);

        const globalAndAlertsData = await getGlobalDashboardData(user.token);
        setGlobalSummary(globalAndAlertsData.globalSummary);
        setActiveAlerts(globalAndAlertsData.activeAlerts);

        const insightsData = await getTrendingInsights(user.token);
  ...
  const uniqueEventTypes = getUniqueEventTypes(trendingInsights);
              ...
              <div className="profile-dropdown-menu">
                <div className="dropdown-profile-header">
                  ...
                  <div className="dropdown-profile-info">
                    <p className="dropdown-profile-name">{user?.name || partnerName}</p>
